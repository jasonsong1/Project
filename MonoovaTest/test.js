...
function RenderReportV6() {
  if (!reportV6EndDate.value) {
    ...
    ReportV6(
      `${baseURL}/receivables/v6/report/${reportV6Startdate.value}?endDate=${reportV6EndDate.value}`
    );
  }
}
...
async function GetAllWebhooks() {
  ...
  let res = await fetch(`${baseURL}/subscriptions/v1/list`, {
    method: "get",
    headers: {
...
function RenderNPPPayment() {
  ...
  fetch(`${baseURL}/financial/v2/transaction/execute`, {
    ...
    .then((result) => {
      ...
      // if (result.statusDescription === "Operation completed successfully") {
      //     result.statusDescription = "rwerewrwe";
      //   }
...
<div>{</div>
  ...
  mobileUI.innerHTML = `    
  <div class="mobileUI mobilereceipt">
      <img src="/images/Monoovaimage.png" class="mobileimagelogo">
     
      <h2 class="text-center mt-2">Payment success!</h2>

       
       <div class="MobileReceitSummary">
          <br>Payment summary
          <br>Amount:$${EnteredAmountValue}
          <br>Reference:${LodgementReferenceValue}
          <br>Date/Time:${dateTime}
          <br>Account Name:${AccountNameValue}
          <br>Description:${DescriptionValue}

         </div>
         <p3 class="purplefont">INVOICE PAID</p3>
...
let DirectDebitPayloadSubmit = document.getElementById(
  "DirectDebitPayloadSubmit"
);
...
//create Automatcher
let CreateAutomatcherPayloadSubmit = document.getElementById(
  "CreateAutomatcherPayloadSubmit"
);

CreateAutomatcherPayloadSubmit.addEventListener(
  "click",
  RenderCreateAutomatcher
);

function CreateAutomatcherVariables() {
  let CreateAutomatcherfields = document.querySelector(
    "#CreateAutomatcherfields"
  );
  let AllCreateAutomatcherfields =
    CreateAutomatcherfields.querySelectorAll("section");
...
function RenderCreateAutomatcher() {
  ...
  fetch(`${baseURL}/receivables/v1/create`, {
    ...
    .then((result) => {
      console.log("API response:", result);
      // Handle the response from the API here
      let resultIniArea = document.getElementById(
        "CreateAutomatcherResultArea"
      );
      resultIniArea.innerHTML = `
<div class="responseBody">
<br>
...
//Set Account Status
let SetACStatusPayloadSubmit = document.getElementById(
  "SetACStatusPayloadSubmit"
);
...
let GetACStatbyACResultArea = document.getElementById(
  "GetACStatbyACResultArea"
);
...
let GetACStatbyUniqueIDResultArea = document.getElementById(
  "GetACStatbyUniqueIDResultArea"
);
let GetACStatbyUniqueIDSubmit = document.getElementById(
  "GetACStatbyUniqueIDSubmit"
);

GetACStatbyUniqueIDSubmit.addEventListener("click", renderGetACStatbyUniqueID);

async function renderGetACStatbyUniqueID() {
  const GetACStatbyUniqueIDAPIKey = document.getElementById(
    "GetACStatbyUniqueIDAPIKey"
  );
  const GetACStatbyUniqueIDNumber = document.getElementById(
    "GetACStatbyUniqueIDNumber"
  );
